...
public abstract class MeshObject
{
    ...
    protected Buffer fillBufferFloatFromString(String[] array)
    {
        ByteBuffer bb = ByteBuffer.allocateDirect(4 * array.length);
        bb.order(ByteOrder.LITTLE_ENDIAN);
        for (String d : array)
            bb.putFloat((float) Float.parseFloat(d));
        bb.rewind();

        return bb;
    }

    protected Buffer fillBufferShortFromString(String[] array)
    {
        // Each short takes 2 bytes
        ByteBuffer bb = ByteBuffer.allocateDirect(2 * array.length);
        bb.order(ByteOrder.LITTLE_ENDIAN);
        for (String s : array)
            bb.putShort( Short.parseShort(s));
        bb.rewind();
